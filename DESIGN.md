...
Retrieval over academic papers. Active; blocked on chunking losing table context.
...
Digests are cached against a `source_hash` (hash of `PROJECT.md` contents + git
HEAD SHA + dirty-state flag). `apex sync` regenerates only projects whose hash
changed, which keeps a full sync cheap and fast once warm.
...
- **`apex doctor` mutates.** It applies pending migrations rather than only
  reporting them, since it is the bootstrap command and §7 says "migrated forward
  on startup". Defensible for M1, surprising for an inspection command. Revisit in
  M2 when real commands exist to carry the migration.
